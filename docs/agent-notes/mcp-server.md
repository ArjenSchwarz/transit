...
- **Always validate enum filter values in query handlers.** When a handler accepts `status`, `not_status`, or `type` as filter parameters, validate each value against the enum's `allCases` before using them. Invalid values must return `isError: true` with a message listing valid options — never silently filter to empty results. Helper methods: `validateTaskStatusFilter(_:key:)` for TaskStatus, `validateMilestoneStatusFilter(_:)` for MilestoneStatus (T-732).
