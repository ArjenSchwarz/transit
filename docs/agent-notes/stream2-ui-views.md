...
- **Clearing description (T-854)**: `TaskService.updateTask` follows the same pattern as `MilestoneService.updateMilestone` — pass `description: nil, clearDescription: true` to nil out an existing description. An explicit non-nil `description` always wins over the flag. `TaskEditView.save()` sets `clearDescription: trimmedDesc.isEmpty` so an emptied field clears the stored value.
