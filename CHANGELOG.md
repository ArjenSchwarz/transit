...
- T-1825: Dashboard milestone filters now retain selected Done or Abandoned milestones within the active project scope, so their filter rows remain visible, distinctly labelled, and individually deselectable. Open options retain their order and terminal rows are deduplicated by UUID; inaccessible or deleted selections still leave the menu available for Clear. Add Task remains open-only.
