...
- T-2018: Task editing now requires the selected project UUID to resolve to a live project model before Save. A project removed while the editor is open disables Save and presents a retryable error rather than dismissing after silently retaining the old project. The task-edit applier also fails closed before any mutation when a changed project has no resolved model, while unchanged project fields still accept nil and project-move milestone clearing/validation remain intact.
