...
- MCP server start, stop, and same-port restart now run through one async desired-state lifecycle coordinator (T-1826). Teardown cancels and awaits Hummingbird's old service task before rebinding, rapid requests coalesce to the latest state, and Settings uses cancellable async requests plus an explicit restart operation instead of synchronous stop/start composition. Live loopback regressions cover listener release, same-port restart, and rapid off/on without `EADDRINUSE`.
