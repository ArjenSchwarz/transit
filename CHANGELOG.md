...
### Fixed
- T-1608: MCP `query_tasks` now returns the exact tool error `Failed to fetch milestones: <error>` when resolving an unscoped milestone-name filter cannot fetch milestones, rather than reporting a false successful `[]`. Legitimate no-match arrays, cross-project same-name aggregation, project-scoped milestone lookup and T-1938 ambiguity handling, malformed-filter precedence, response shape, and later task-fetch errors are unchanged; deterministic MCP coverage injects the failed milestone fetch and asserts the exact error.
